...
class TransactionController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        try {
            DB::transaction(function () use ($request) {
                ...
                // OPTIMASI: Insert detail transaksi sekaligus dalam 1x query
    ...
    public function history(Request $request)
    {
        ...
        $totalOmzet = $this->calculateTotalOmzet(
            $month,
            $year
        );
        $bestSeller = $this->getBestSellerByMonthYear(
            $month,
            $year
        );
     ...
     */
    private function calculateTotalOmzet(
        int $month,
        int $year
    ): int {
        return (int) Transaction::where(
            'user_id',
            auth()->user()->id
        )
            ->byMonthYear(
     ...
     */
    private function getBestSellerByMonthYear(
        int $month,
        int $year
    ) {
    ...
    private function calculateTotalProductsSold(
        ...
        return (int) DetailTransaction::whereHas(
            'transaction',
            function ($query) use ($month, $year) {
                $query->where('user_id', auth()->user()->id)
                    ->byMonthYear($month, $year);

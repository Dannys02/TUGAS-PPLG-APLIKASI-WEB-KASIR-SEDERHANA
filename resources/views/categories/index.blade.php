        ...
        <div class="lg:col-span-1">
            <div class="bg-white border border-slate-200 rounded-2xl shadow-sm p-6">
                <h2 class="text-base font-bold text-slate-800 mb-5 pb-3 border-b border-slate-100 flex items-center gap-2">
                    <i
                        class="fa-solid {{ isset($editCategory) ? 'fa-pen-to-square text-amber-500' : 'fa-plus text-blue-500' }}"></i>
                ...
                <form
                    ...
                    <div class="flex flex-col gap-2.5 pt-2">
                        <button type="submit"
                            class="w-full flex items-center justify-center gap-2 px-4 py-3 text-white {{ isset($editCategory) ? 'bg-amber-500 hover:bg-amber-600 shadow-amber-500/10' : 'bg-blue-600 hover:bg-blue-700 shadow-blue-500/10' }} font-semibold text-sm rounded-xl shadow-sm transition-colors">
                        ...
                        @if (isset($editCategory))
                            <a href="{{ route('categories.index') }}"
                                class="w-full flex items-center justify-center px-4 py-3 border border-slate-200 text-slate-600 bg-white hover:bg-slate-50 font-semibold text-sm rounded-xl transition-colors text-center">
                                Batal Edit
                            </a>
                        @endif
                    </div>
                        ...
                        <tbody class="divide-y divide-slate-100 text-sm">
                            @forelse($categories as $c)
                                <tr class="hover:bg-blue-50/30 transition-colors group">
                                    <td class="py-3.5 px-6 text-center text-slate-400 font-medium text-xs">
                                        {{ $loop->iteration }}</td>
                                    <td
                                    ...
                                    <td class="py-3.5 px-6 text-center">
                                        <div class="flex items-center justify-center gap-1.5">
                                            ...
                                            <form action="{{ route('categories.destroy', $c->id) }}" method="POST"
                                                onsubmit="confirmDelete(event, this, 'Apakah Anda yakin ingin menghapus kategori {{ addslashes($c->nama_kategori) }}?')"
                                ...
                                <tr>
                                    <td colspan="3" class="py-16 text-center">
                                        <div class="flex flex-col items-center justify-center max-w-sm mx-auto">
                                            ...
                                            <p class="text-xs text-slate-400 mt-1">Sistem belum mendeteksi adanya kategori
                                                produk. Gunakan formulir di sebelah kiri untuk menambahkannya.</p>
                                        </div>
                                    </td>
                                </tr>
                            @endforelse

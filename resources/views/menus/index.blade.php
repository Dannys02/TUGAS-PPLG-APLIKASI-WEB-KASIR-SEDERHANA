...
@section('content')
    <div class="flex items-center gap-3 mb-8">
        ...
        <div>
            ...
            <p class="text-sm text-slate-500">Kelola daftar produk, harga jual, konfigurasi kategori, dan ketersediaan stok kasir</p>
        </div>
    </div>
    ...
    <div class="bg-white border border-slate-200 rounded-2xl shadow-sm p-6 mb-8">
        ...
        <form action="{{ isset($editMenu) ? route('menus.update', $editMenu->id) : route('menus.store') }}" method="POST">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5">
                ...
                <div class="space-y-2">
                    ...
                    <div class="relative flex items-center">
                        ...
                        <select name="kategori_id" class="w-full pl-11 pr-10 py-3 border border-slate-200 rounded-xl bg-slate-50 text-slate-800 text-sm focus:outline-none focus:border-blue-500 focus:bg-white focus:ring-4 focus:ring-blue-500/10 transition-all appearance-none cursor-pointer" required>
                ...
                <div class="space-y-2">
                    ...
                    <div class="relative flex items-center">
                        <span class="absolute left-4 text-slate-500 font-semibold text-xs bg-slate-200/60 px-2 py-1 rounded-md select-none">
                            Rp
                        </span>
            ...
            <div class="flex flex-col sm:flex-row gap-3 mt-6 justify-start">
                <button type="submit" class="w-full sm:w-auto flex items-center justify-center gap-2 px-6 py-3 text-white {{ isset($editMenu) ? 'bg-amber-500 hover:bg-amber-600 shadow-amber-500/10' : 'bg-blue-600 hover:bg-blue-700 shadow-blue-500/10' }} font-semibold text-sm rounded-xl shadow-sm transition-colors">
                    <i class="fa-solid {{ isset($editMenu) ? 'fa-floppy-disk' : 'fa-plus' }}"></i>
                    {{ isset($editMenu) ? 'Simpan Perubahan' : 'Tambah Menu' }}
                </button>

                @if (isset($editMenu))
                    <a href="{{ route('menus.index') }}" class="w-full sm:w-auto flex items-center justify-center px-6 py-3 border border-slate-200 text-slate-600 bg-white hover:bg-slate-50 font-semibold text-sm rounded-xl transition-colors text-center">
                        Batal Edit
                    </a>
                @endif
            </div>
        ...
        <div class="p-5 border-b border-slate-100 bg-slate-50/50 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            ...
            <form method="GET" action="{{ route('menus.index') }}" class="flex items-center gap-2 w-full sm:w-auto">
                <div class="relative flex-grow sm:flex-grow-0 sm:w-64 flex items-center">
                    ...
                    <input type="text" name="search" placeholder="Cari masukan nama menu..." value="{{ request('search') }}"
                ...
                @if(request()->filled('search'))
                    <a href="{{ route('menus.index') }}" class="bg-red-50 hover:bg-red-100 text-red-600 px-3 py-2 rounded-xl transition-colors text-xs" title="Reset Pencarian">
                ...
                <tbody class="divide-y divide-slate-100 text-sm">
                    @forelse($menus as $m)
                        <tr class="hover:bg-blue-50/30 transition-colors group">
                            ...
                            <td class="py-3.5 px-6 font-semibold text-slate-700 group-hover:text-blue-600 transition-colors">{{ $m->nama_menu }}</td>
                            <td class="py-3.5 px-6">
                                <span class="inline-flex items-center px-2.5 py-1 rounded-lg text-xs font-semibold bg-blue-50 text-blue-700 border border-blue-100/60 shadow-sm">
                                    {{ $m->category->nama_kategori ?? 'Umum' }}
                                </span>
                            </td>
                            <td class="py-3.5 px-6 font-bold text-slate-800 tracking-tight">
                                <span class="text-xs font-normal text-slate-400 mr-0.5">Rp</span>{{ number_format($m->harga, 0, ',', '.') }}
                            </td>
                            <td class="py-3.5 px-6 text-center">
                                @if ($m->stok > 10)
                                    <span class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-bold bg-green-50 text-green-700 border border-green-200">
                                        <span class="w-1.5 h-1.5 rounded-full bg-green-500 animate-pulse"></span> {{ $m->stok }}
                                    </span>
                                @elseif($m->stok > 0)
                            ...
                            <td class="py-3.5 px-6 text-center">
                                <div class="flex items-center justify-center gap-1.5">
                                    <a href="{{ route('menus.edit', $m->id) }}" class="p-2 text-amber-600 bg-amber-50 border border-amber-100 hover:bg-amber-100 rounded-lg transition-colors" title="Ubah Menu">
                                    ...
                                    <form action="{{ route('menus.destroy', $m->id) }}" method="POST"
                                        ...
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="p-2 text-red-600 bg-red-50 border border-red-100 hover:bg-red-100 rounded-lg transition-colors" title="Hapus Item">
                        ...
                        <tr>
                            <td colspan="6" class="py-16 text-center">
                                <div class="flex flex-col items-center justify-center max-w-sm mx-auto">
                                    ...
                                    <p class="text-xs text-slate-400 mt-1">Sistem belum mendeteksi adanya data menu produk kuliner Anda. Gunakan form di atas untuk memasukkan menu baru.</p>
                                </div>
                            </td>
                        </tr>
                    @endforelse
